...
All three metrics are evaluated at the level of individual spike events. Let $S_{k,j}=1$ indicate that the $j$th spike in time bin $k$ occurred, and let $y_{k,j}$ denote the mark of that event: the neuron identity for a spike-sorted model or the waveform features for a clusterless model. For the spike-sorted models analyzed here, HPD overlap and KL divergence compare the one-step predictive density
\begin{equation*}
    P_k(x_k) = p(x_k \mid y_{1:k-1})
\end{equation*}
with the normalized one-spike likelihood
\begin{equation}
    \label{eq:single_spike_likelihood}
    Q_{k,j}(x_k)
    = \frac{\operatorname{Poisson}\!\left(1;\, \lambda_{y_{k,j}}(x_k)\right)}
    {\int \operatorname{Poisson}\!\left(1;\, \lambda_{y_{k,j}}(u)\right)\,du},
\end{equation}
where $\lambda_c(x_k)$ is the model-implied expected spike count for neuron $c$ in time bin $k$ at state $x_k$. If $r_c(x_k)$ denotes the corresponding conditional intensity and $\Delta t$ is the bin width, then $\lambda_c(x_k)=r_c(x_k)\Delta t$; throughout, we use $\lambda_c$ for this per-bin Poisson mean. Thus, $Q_{k,j}$ is the likelihood contributed by the observed spike event alone, rather than the joint likelihood of all spike counts in the bin. If several spikes occur in one bin, each event is evaluated separately against the same one-step predictive density $P_k$. If the observed neuron's expected count is zero at every evaluated state, the denominator in Eq.~\eqref{eq:single_spike_likelihood} vanishes; as in the implementation, we then define $Q_{k,j}$ to be uniform over the state grid because the event carries no positional information under this likelihood. For a clusterless model, the analogous $Q_{k,j}$ is obtained by normalizing the single-event waveform-mark likelihood over the state space.
...
Using the event notation introduced above, the relevant observation distribution is the distribution of the spike's mark given that a spike occurred. For a spike-sorted model with expected bin count $\lambda_c(x_k)$ for neuron $c$, this distribution is
\begin{equation}
...
\end{equation}
Let $\Lambda(x_k)=\sum_{d=1}^{C}\lambda_d(x_k)$ denote the total population intensity. At a known spike time, observing that a spike occurred also provides information about the latent state: states with higher total intensity are more likely to have generated the event. The appropriate state distribution for predicting its mark is therefore
\begin{equation}
    \label{eq:event_conditioned_predictive}
    P_k^{\mathrm{event}}(x_k)
    = \frac{\Lambda(x_k)P_k(x_k)}{\int \Lambda(u)P_k(u)\,du}.
\end{equation}
Consequently, the predictive probability of cell identity is obtained by integrating the raw cell intensities over the one-step prediction distribution and then normalizing across cells:
\begin{equation}
...
\end{equation}
The order of these operations matters: averaging the state-conditional fractions $\pi_c(x_k)$ directly under $P_k$ is generally different and is equivalent only when $\Lambda(x_k)$ is constant over the support of $P_k$.

For a clusterless model, let $\lambda(y,x_k)$ be the marked intensity density for waveform mark $y$, so that $\Lambda(x_k)=\int \lambda(v,x_k)\,dv$. The corresponding predictive mark density is
\begin{equation}
    \label{eq:fpred}
    f_{\mathrm{pred}}(y)
...
Finally, one of the most well-known measures of discrepancy between probability distributions is the Kullback--Leibler (KL) divergence~\cite{kullbackInformationSufficiency1951}, defined in this case as the expected excess logarithmic loss incurred when the normalized one-spike likelihood is used to approximate the predictive distribution, with the expectation taken with respect to the predictive distribution:
\begin{equation}
    \label{eq:kl}
...
\begin{figure}[htbp]
    ...
    \caption{Computation of the three local goodness-of-fit metrics. \textbf{(a)} For HPD overlap, high-density regions are computed for the predictive distribution (top) and the normalized one-spike likelihood (middle). The volume of their intersection is then divided by the volume of the smaller region (bottom). \textbf{(b)} The conceptual sampling construction for the predictive check: state values are sampled from the predictive distribution (top), and each sampled state is used to generate the identity of a neuron that could have produced the spike (middle). The metric is the probability that the log predictive probability of the observed spike is greater than or equal to that of a replicated spike (bottom); for the finite spike-sorted observation spaces analyzed here, this probability is evaluated exactly rather than by sampling. \textbf{(c)} The KL divergence compares the predictive distribution (blue) and the one-spike likelihood (orange) (top) by computing the difference in their log density (middle) and computing its expectation with respect to the predictive distribution (bottom).}
    \label{fig:metrics}
\end{figure}
