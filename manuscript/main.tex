...
\end{equation}
If $r_c(x_k)$ denotes the corresponding conditional intensity and $\Delta t$ is the bin width, then $\lambda_c(x_k)=r_c(x_k)\Delta t$; the common $\Delta t$ cancels in Eq.~\eqref{eq:single_event_likelihood}. Thus, $Q_{k,j}$ contains only the likelihood factor attributable to the selected event. It deliberately excludes the bin-level exposure term, silent neurons, and total spike count, which cannot be assigned to one particular event. Those aspects of fit require a complementary count- or time-rescaling diagnostic. Using $\operatorname{Poisson}(1;\lambda)=\lambda e^{-\lambda}$ for every event would attach a no-additional-spike factor to each event and would count that exposure repeatedly when multiple spikes share a bin. If several spikes occur in one bin, each event is instead evaluated using its own intensity factor against the same one-step predictive density $P_k$; events from the same neuron therefore receive identical local values because the binned model holds $P_k$ and $\lambda_c$ constant within the bin. If the observed neuron's expected count is zero at every evaluated state, the denominator in Eq.~\eqref{eq:single_event_likelihood} vanishes. In that case $Q_{k,j}$ is undefined, and the implementation raises an error rather than assigning an arbitrary distribution. For a clusterless model, the analogous $Q_{k,j}$ is obtained by normalizing the marked event-intensity density over the state space.
