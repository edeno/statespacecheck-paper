...
The 32-second simulation ($T = \num{32000}$ steps) is partitioned into eight contiguous phases: an opening baseline, four misfit windows, and three clean-recovery interludes that restore the matched-model configuration. Each misfit perturbs either the generative process or the decoder:
...
\begin{description}
    ...
    \item[\textbf{Wide-dynamics noise} (steps \num{30000}--\num{32000}; decoder side):] the decoder is given an inflated transition matrix with step standard deviation $\sigma_{\mathrm{inflated}} = 20$ a.u.\ (forty times the baseline), so its one-step predictive spreads broadly across the track; spike generation is unchanged. This phase is engineered to inflate the KL divergence while leaving HPD overlap and the rank-based predictive $p$-value near baseline.
\end{description}
