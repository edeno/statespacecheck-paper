...
\end{equation}
where $\phi(\cdot \mid \mu, \sigma^{2})$ is the Gaussian density and $\alpha = \SimRateScale$ is a rate-scale parameter. The peak expected count is $\alpha / (\sigma_{\mathrm{pf}}\sqrt{2\pi}) \approx \SimPeakCountPerStep$ spikes per step, corresponding to a peak rate of approximately \SI{\SimPeakRateHz}{Hz}. Spike counts are drawn independently across cells at each step, $y_{c,t} \sim \mathrm{Poisson}\!\left(\lambda_c(x_t)\right)$. Except during the replay control described below, $x_t$ is both the animal's physical position and the position represented by the simulated ensemble.
...
\begin{description}
    ...
    \item[\textbf{History-dependent firing}:] During steps \num{\SimRecoveryOneEnd}--\num{\SimHistoryEnd}, the generative-side observation model is perturbed. Spiking is generated with a \SI{\SimRefractoryMs}{ms} hard refractory period followed by a \SimBurstStartMs--\SI{\SimBurstEndMs}{ms} burst window with a \SimBurstFactorWord{}fold rate increase, matching the rough phenomenology of CA1 pyramidal-cell refractory and burst windows; the decoder still assumes independent Poisson firing. The per-spike spatial likelihood is unchanged, so the misfit lives in the spike-train temporal joint distribution.
