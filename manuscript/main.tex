...
Despite the wide use of state space models in neuroscience, methods to validate them remain underdeveloped and underused. When validation is skipped or reduced to a single pass/fail verdict, a model that fails in one behavioral epoch but succeeds elsewhere is accepted or rejected wholesale, and the specific assumption responsible for a misfit is never identified---so the model is never refined in a targeted way. One reason validation lags is that latent states are inherently difficult to evaluate~\cite{newmanModellingPopulationDynamics2014}: in the absence of ground truth, model validity can only be inferred indirectly, for example by comparing relative likelihoods across models, the quality of reconstructed signals, or performance on downstream tasks. Neural data are also dynamic, so model fit can vary substantially over time. Compounding this, the most common validation tools---cross-validation and ``shuffle'' permutation tests---summarize the entire recording in a single number: they report whether a model generalizes better than chance or better than a simpler alternative, but not \emph{when} during a session the model fits poorly or \emph{which} of its components is responsible. Consequently, these methods can reject intentionally simplified yet scientifically meaningful models while offering little guidance for improving them. For example, analyses to decode rat movements from hippocampal place cells typically ignore known features of the rat's movement (such as momentum) and known features of neurons (such as refractoriness and bursting). These omissions can produce detectable model mismatch even when the resulting decoding trajectories remain scientifically informative~\cite{DengClusterlessDecodingPosition2015,Taocommongoodnessoffitframework2018,zengStateSpaceFrameworkCausal2026}.

State space models already contain rich diagnostic information in the form of distributions used to estimate the state process at each time: the normalized likelihood, which reflects information from only the current neural observation, and the prediction distribution, which aggregates information from all past observations through the state-space model structure. One potential source of targeted, temporally local diagnostic information is a direct comparison of these two distributions. We do not expect them to be identical. The prediction distribution aggregates information from many past spikes, whereas the likelihood reflects only the current observation, so the two carry different amounts of information about the state and will typically differ in shape and spread.
...
\end{equation}
The common factor $\Delta t$ cancels in this normalization. $Q_{k,j}$ contains only the likelihood factor attributable to the selected event. It deliberately excludes the bin-level exposure term, silent neurons, and total spike count, which cannot be assigned to one particular event. Those aspects of fit require a complementary count- or time-rescaling diagnostic. If several spikes occur in one bin, each event is evaluated separately against the same one-step predictive density $P_k$. For a clusterless model, the analogous $Q_{k,j}$ is obtained by normalizing the marked event-intensity density over the state space.
...
\end{equation}
where $\mathrm{HPD}_{P_k}$ and $\mathrm{HPD}_{Q_{k,j}}$ are the highest probability-density regions of the predictive density and the normalized single-event likelihood, respectively, and $|\cdot|$ denotes the volume of the region. The numerator is the size of the overlap between these regions and the denominator is the size of the smaller of the two HPD regions. This ratio is the Szymkiewicz--Simpson overlap coefficient applied to the two HPD regions. Unlike the Jaccard index, which divides by the size of the union and therefore penalizes any difference in region size, the overlap coefficient equals 1 whenever one region is nested inside the other, which is exactly the nesting-tolerant behavior our consistency criterion requires. This normalization ensures that the resulting value is in the range $[0,1]$, with 0 indicating no overlap, and 1 indicating that one region is entirely contained within the other. For example, if the likelihood region is wider and therefore less informative than the predictive region but completely contains it then the HPD ratio will be 1, suggesting that the two distributions are consistent under this criterion. Conversely, if the ratio is close to 0, it indicates little or no overlap, suggesting significant differences between the range of values for the state that would be inferred from these two distributions. Throughout, we compute 95\% HPD regions.
...
\end{equation}
where $\Delta t=\SI{1}{ms}$, $\phi(\cdot \mid \mu, \sigma^{2})$ is the Gaussian density, and $\alpha = \SimRateScale$ is a scale parameter for the expected count. The peak expected count is $\alpha / (\sigma_{\mathrm{pf}}\sqrt{2\pi}) \approx \SimPeakCountPerStep$ spikes per step, corresponding to a peak rate of approximately \SI{\SimPeakRateHz}{Hz}. Spike counts are drawn independently across cells at each step, $y_{c,t} \sim \mathrm{Poisson}\!\left(m_c(x_t)\right)$. Except during the replay control described below, $x_t$ is both the animal's physical position and the position represented by the simulated ensemble.

\SimNSparseCellsWordCap{} additional cells form a sparse population of narrow fields clustered near $x=\SimSparsePosition$ a.u.\ (centers spread over $\SimSparseSpreadLow$--$\SimSparseSpreadHigh$ a.u., each with standard deviation $\SimSparseFieldStd$ a.u.). Each fires as an independent Poisson process. Its per-cell peak rate is \SI{\SimSparseBaselineRateHz}{Hz} during the baseline and recovery periods and \SI{\SimSparseActiveRateHz}{Hz} during the final sparse-population control; with \SimNSparseCellsWord{} cells the aggregate rate stays low (a few spikes per second), so the spikes remain temporally isolated. The decoder uses these same epoch-specific rates, so recruiting these cells does not itself create an observation-model mismatch.

\paragraph{Position dynamics.}
During baseline windows, the animal's position evolves as a Gaussian random walk with step standard deviation $\sigma_{\mathrm{pred}} = \SimPredictionStepStd$ a.u. The trajectory is reflected at $\SimPositionMin$ and $\SimTrackLength$ a.u.\ throughout the session, starts at $x_0 = 0$, and is continuous across phases: the final position of each phase becomes the initial position of the next.

\paragraph{Phase schedule.}
The \SimDurationSeconds-second simulation ($T = \num{\SimDurationSteps}$ steps) is partitioned into \SimNPhasesWord{} contiguous phases: an opening baseline, three misfit windows, three clean-recovery interludes, and a final sparse-population control. Each misfit window perturbs the observation model $p(y_k \mid x_k)$ or the state-transition model $p(x_k \mid x_{k-1})$ by altering either the generative process or the decoder's assumptions:

\begin{description}
    \item[\textbf{Remap}:] During steps \num{\SimRemapStart}--\num{\SimRemapEnd}, the decoder-side observation model is perturbed. The decoder evaluates per-spike likelihoods against a globally remapped set of place-fields: the \SimNPlaceCellsWord{} place-field centers are scrambled by a fixed random permutation (a derangement in which every field is displaced by at least \SimRemapDisplacementWord{} positions), modeling global remapping to statistically independent locations. Both the decoder's posterior update and its per-spike diagnostics use this remapped likelihood---the diagnostics are evaluated against the model the decoder actually uses, not the true generative fields---so the misfit surfaces from the scramble's spatial incoherence rather than from any oracle knowledge of the correct fields. Spike generation is unchanged.
    \item[\textbf{History-dependent firing}:] During steps \num{\SimRecoveryOneEnd}--\num{\SimHistoryEnd}, the generative-side observation model is perturbed. Spike counts are generated with each cell's Poisson mean set to zero for \SI{\SimRefractoryMs}{ms} after a step in which it spiked, followed by a \SimBurstStartMs--\SI{\SimBurstEndMs}{ms} burst window with a \SimBurstFactorWord{}fold rate increase, matching the rough phenomenology of CA1 pyramidal-cell refractory and burst windows; the decoder still assumes independent Poisson firing. The per-spike spatial likelihood is unchanged, so the misfit lives in the spike-train temporal joint distribution.
...
Fig~\ref{fig:simulation}b shows the percentage of spikes flagged by each metric in each period, summarized by the median across \SimNRealizations{} independent realizations of the simulation. A spike is flagged whenever its diagnostic crosses the corresponding threshold in the direction of worse fit: at or below the \SimHpdPercentile-percentile baseline value for HPD overlap, at or below the fixed $\SimPredictiveCutoff$ cutoff for the rank-based predictive $p$-value, or at or above the \SimKlPercentile-percentile baseline value for KL divergence. The median percentage flagged during the remapping period is substantial for all three metrics (\SimRemapFlagMin--\SimRemapFlagMax\%), demonstrating that all three methods are able to identify inconsistent spiking information leading to incorrect state estimates; the percentage nonetheless varies from realization to realization because it depends on how much of the scrambled map the simulated trajectory visits. All three metrics flag only \SimHistoryFlagMin--\SimHistoryFlagMax\% of spikes during the history-dependence period. The replay control is flagged at a low rate---a median of \SimReplayFlagMin--\SimReplayFlagMax\% across the three metrics, comparable to the well-specified baseline---confirming that a large decoded-versus-physical-position divergence is not, by itself, registered as model misspecification. The drift period is flagged more often (\SimDriftFlagMin--\SimDriftFlagMax\%), most notably by the rank-based predictive $p$-value, but still far less than during the remapping period. In the sparse-population control, HPD overlap and the rank-based predictive $p$-value flag no spikes, whereas KL divergence flags a median of \SimSparseKlFlag\%. This is consistent with the realization shown in Fig~\ref{fig:simulation}a, where HPD overlap and the predictive check registered the spike and the prediction as consistent, while KL responded to the difference in concentration between the predictive distribution and likelihood despite them carrying consistent information about the state process.

The decoding-error row of Fig~\ref{fig:simulation}b places these flag percentages against ground truth. In the well-specified windows the median absolute error of the filtered posterior mean is \SimWellSpecifiedError{} a.u., less than one fifth of a place-field width. Remapping raises it to \SimRemapError{} a.u., matching the high flag percentages. History-dependent firing leaves it essentially unchanged (\SimHistoryError{} a.u.), consistent with its low flag percentages: this misfit alters the temporal structure of the spike train without displacing the spatial information each spike carries. The drift period, by contrast, exposes a limitation of event-level diagnostics. The median error rises to \SimDriftError{} a.u., more than four times the well-specified value, because the filtered posterior lags the fast-moving position, yet only \SimDriftFlagMin--\SimDriftFlagMax\% of spikes are flagged. Because each spike is compared with a predictive distribution that the preceding spikes have already pulled toward the lagged estimate, a persistent lag produces a modest per-spike disagreement rather than a large one. During the replay control the error relative to the animal's physical position is large by construction (\SimReplayError{} a.u.) because the decoder is tracking the represented trajectory. The sparse-population control is decoded accurately (\SimSparseError{} a.u.) even though KL divergence flags \SimSparseKlFlag\% of its spikes, confirming that those flags do not correspond to decoding errors.
...
The full recording clarifies what these rescues represent (Fig~\ref{fig:realdata_supplement}). Fewer HPD flags under the Continuous--Fragmented model do not by themselves show that it estimates the state better. Because HPD overlap divides the shared region by the smaller of the two regions, a prediction whose region contains the likelihood's region scores as fully consistent, however broad it is. Across all spike events, the median 95\% predictive region spans \RecContRegionAll{} of \RecNPositionBins{} position bins under the Continuous model and \RecFragRegionAll{} bins under the Continuous--Fragmented model; at the rescued events the medians are \RecContRegionRescued{} and \RecFragRegionRescued{} bins (Fig~\ref{fig:realdata_supplement}a). The likelihood's 95\% region is itself broad (median \RecLikelihoodRegion{} bins), so a prediction is flagged only when its region lies almost entirely in the few bins outside it, which a broad prediction cannot do. To measure how many flags broadening alone removes, we replaced the Continuous model's prediction at every event by the mixture $(1-w)P_k + wU$, with $U$ uniform over the track, and recomputed HPD overlap on the same events (Fig~\ref{fig:realdata_supplement}b). A weight of $w=\RecMixWeightMin$ removes \RecMixRemovedMin\% of the Continuous model's HPD flags, and $w=\RecMixWeightMatch$ removes \RecMixRemovedMatch\%, at least the \RecHpdRescuedPercent\% rescued by the fitted Continuous--Fragmented model. The mixture alters the diagnostic's input rather than refitting a decoder, so it does not show that the Continuous--Fragmented model fails to improve the state estimate; it shows that the rescue count alone cannot distinguish a better model from a broader prediction. The rescue also depends on the coverage level: at 80\% and 50\% coverage, the Continuous--Fragmented model rescues \RecCovEightyRescuedPercent\% and \RecCovFiftyRescuedPercent\% of the Continuous model's HPD flags.

The rank-based predictive $p$-value flags differ in kind. Units firing below \SI{1}{Hz} over the session contribute \RecUnderOneHzEventsPercent\% of spike events but \RecUnderOneHzFlagsPvalueCont\% of the Continuous model's rank-based flags (Fig~\ref{fig:realdata_supplement}c,d), consistent with the definition of the check, under which a spike from a rarely firing unit is an improbable mark. Continuous-model HPD flags are more frequent during immobility than during movement (\RecImmobileHpdCont\% versus \RecMovingHpdCont\% of spike events, with head speed below or above \SI{\RecSpeedCutoff}{cm/s}), consistent with nonlocal representations during rest, whereas rank-based flags occur at similar rates in both (\RecImmobilePvalueCont\% versus \RecMovingPvalueCont\%; Fig~\ref{fig:realdata_supplement}e).
...
\begin{figure}[htbp]
    ...
    \caption{Full-recording characterization of the Fig~\ref{fig:realdata} flags. \textbf{(a)} Cumulative distributions of the 95\% predictive HPD-region size (in track-interior bins, of \RecNPositionBins) at every spike event (solid) and at the events rescued by the Continuous--Fragmented model (dashed), for the Continuous (blue) and Continuous--Fragmented (orange) models, with the likelihood's 95\% region size (dotted) for comparison. \textbf{(b)} Fraction of the Continuous model's HPD flags removed when its prediction is replaced by the uniform mixture $(1-w)P_k+wU$ on the same events, as a function of $w$, at 95\% (solid), 80\% (dashed), and 50\% (dotted) coverage; horizontal lines mark the fraction rescued by the fitted Continuous--Fragmented model at each coverage. The mixture is a counterfactual applied to the diagnostic's input, not a refitted decoder. \textbf{(c)} Each active unit's session firing rate against the fraction of its spike events flagged by the rank-based $p$-value (filled) and by HPD overlap (open), for both models; the dotted line marks \SI{\RecLowRateThresholdHz}{Hz}. \textbf{(d)} Fractions of active units, of spike events, and of the Continuous model's rank-based and HPD flags contributed by each firing-rate group. \textbf{(e)} Percentage of spike events flagged during immobility and movement (head speed below or above \SI{\RecSpeedCutoff}{cm/s}) by metric and model. No independent replay labels are available for this recording.}
