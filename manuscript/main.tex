...
The simulation runs a Bayesian filter over a 38-second session in which the generative process and the decoder coincide except inside five misfit windows; each window perturbs one specific component of the matched model so the resulting metric responses can be attributed to a known fault.
...
\end{equation}
where $\phi(\cdot \mid \mu, \sigma^{2})$ is the Gaussian density and $\alpha = 5$ is a peak-rate scale factor. With a 1\,ms simulation step, the peak rate is $\alpha / (\sigma_{\mathrm{pf}}\sqrt{2\pi}) \approx 200$\,Hz, within the range observed at the place-field centers of highly active hippocampal pyramidal cells. Spike counts are drawn independently across cells at each step, $y_{c,t} \sim \mathrm{Poisson}\!\left(\lambda_c(x_t)\right)$.

\paragraph{Position dynamics.}
In baseline windows the animal's position evolves as a Gaussian random walk with step standard deviation $\sigma_{\mathrm{pred}} = 0.5$ a.u.\ and reflecting boundaries at $0$ and $100$ a.u. The trajectory starts at $x_0 = 0$ and is continuous across phases: the final position of each phase becomes the initial position of the next.

\paragraph{Phase schedule.}
The 38-second simulation ($T = 38{,}000$ steps) is partitioned into ten contiguous phases: an opening baseline, five misfit windows, and four clean-recovery interludes that restore the matched-model configuration. Each misfit perturbs either the generative process, the decoder, or both:

\begin{enumerate}
    \item \textbf{Remap} (steps 6{,}000--10{,}000; decoder side): spike generation is unchanged, but the decoder evaluates per-spike likelihoods against a remapped place-field map in which three cell pairs (0$\leftrightarrow$9, 1$\leftrightarrow$8, 2$\leftrightarrow$7) have swapped centers, so the decoder's intended likelihood is at the wrong location for six of the eleven cells.
    \item \textbf{History-dependent firing} (steps 14{,}000--18{,}000; generative side): spikes are generated with a 1\,ms hard refractory period followed by a 2--10\,ms post-spike burst window in which the rate is multiplied by $3\times$, roughly matching CA1 pyramidal-cell refractory and burst phenomenology; the decoder continues to assume independent Poisson firing. The per-spike spatial likelihood for any individual spike is unchanged, so the misfit lives entirely in the spike-train temporal joint distribution.
    \item \textbf{Drift} (steps 22{,}000--26{,}000; generative side): the trajectory is generated with persistent velocity, $v_t = 0.8\, v_{t-1} + \eta_t$ and $x_t = x_{t-1} + v_t$ with $\eta_t \sim \mathcal{N}(0, \sigma_{\mathrm{pred}}^{2})$; the decoder continues to assume a memoryless random walk with step standard deviation $\sigma_{\mathrm{pred}}$.
    \item \textbf{Wide-dynamics noise} (steps 30{,}000--32{,}000; decoder side): the decoder is given an inflated transition matrix with step standard deviation $\sigma_{\mathrm{inflated}} = 20$ a.u.\ (forty times the baseline), so its one-step predictive spreads broadly across the track; spike generation is unchanged. This phase is engineered to inflate the KL divergence while leaving HPD overlap and the rank-based predictive $p$-value near baseline.
    \item \textbf{Wiggly-flat likelihood} (steps 34{,}000--38{,}000; decoder side): the decoder swaps its per-cell rate functions for a set of nearly position-independent rates,
    \begin{equation}
        r_c(x) = r_0 + \delta\, \sin\!\left(2\pi k\, \frac{x - x_{\min}}{L} + \varphi_c\right),
    \end{equation}
    with baseline $r_0 = 0.05$, modulation amplitude $\delta = 0.01$, $k = 5$ spatial cycles across the track, and cell-specific phase $\varphi_c = 2\pi c / n_{\mathrm{cells}}$; both the posterior update and the diagnostic rate table use these wiggly-flat rates during this window. Spike generation is unchanged.
\end{enumerate}

The four intermediate \textbf{clean-recovery} windows (steps 10{,}000--14{,}000; 18{,}000--22{,}000; 26{,}000--30{,}000; 32{,}000--34{,}000) restore both the generative process and the decoder to the baseline configuration, providing within-session reference periods against which each metric's behavior during a misfit can be calibrated. With this schedule fixed, each diagnostic can be evaluated directly against a known ground-truth fault, and the three responses can be compared across faults that differ in which side of the model is perturbed.
