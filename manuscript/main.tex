...
\caption{Per-spike goodness-of-fit diagnostics on real hippocampal recordings during navigation on a multi-arm maze; data from \citet{Comrie2024}. (a) Spatial layout of the track with the animal's trajectory overlaid in grey (top), and a longer context window showing the surrounding run and immobility periods under the Continuous decoder (below). (b) Zoomed-in detail view of the same context window under the Continuous decoder. (c) The same detail view under a Continuous--Fragmented decoder.}
\label{fig:real_data}
\end{figure}
