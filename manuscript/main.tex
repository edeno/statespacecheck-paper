...
\end{equation}
where $\phi(\cdot \mid \mu, \sigma^{2})$ is the Gaussian density and $\alpha = 5$ is a peak-rate scale factor. With a 1\,ms simulation step, the peak rate is $\alpha / (\sigma_{\mathrm{pf}}\sqrt{2\pi}) \approx 200$\,Hz, in the range commonly assumed for active hippocampal pyramidal cells at their place-field centers. Spike counts are drawn independently across cells at each step, $y_{c,t} \sim \mathrm{Poisson}\!\left(\lambda_c(x_t)\right)$.
...
The 38-second simulation ($T = 38{,}000$ steps) is partitioned into ten contiguous phases: an opening baseline, five misfit windows, and four clean-recovery interludes that restore the matched-model configuration. Each misfit perturbs either the generative process or the decoder:
