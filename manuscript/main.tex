...
To address these gaps, we develop and validate a suite of three complementary goodness-of-fit methods -- KL divergence, highest posterior density (HPD) region overlap, predictive checks -- designed to provide actionable feedback to neuroscientists. Specifically, we use these methods, each built on the mismatch between prediction and likelihood, to: (1) identify time periods of poor model--data agreement; (2) reveal how the three metrics respond selectively to different types of misspecification; and (3) provide intuitive visualizations accessible to a broad audience. We demonstrate the utility of these techniques on simulated and real hippocampal data, offering a path for neuroscientists to move from binary model rejection (is this model good or bad?) toward an iterative, diagnostic approach to building interpretable theories of neural dynamics.
