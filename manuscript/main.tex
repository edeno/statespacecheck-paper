...
Fig~\ref{fig:simulation}b shows the percentage of spikes flagged by each metric in each period, summarized by the median across 100 independent realizations of the simulation. A spike is flagged whenever its diagnostic crosses the corresponding threshold in the direction of worse fit: at or below the 1st-percentile baseline value for HPD overlap, at or below the fixed $0.05$ cutoff for the rank-based predictive $p$-value, or at or above the 99th-percentile baseline value for KL divergence. The median percentage flagged during the remapping period is substantial for all three metrics (37--43\%), demonstrating that all three methods are able to identify inconsistent spiking information leading to incorrect state estimates; the percentage nonetheless varies from realization to realization because it depends on how much of the scrambled map the simulated trajectory visits. All three metrics flag only 1--2\% of spikes during the history-dependence period. The replay control is flagged at a low rate---a median of 1--3\% across the three metrics, comparable to the well-specified baseline---confirming that a large decoded-versus-physical-position divergence is not, by itself, registered as model misspecification. The drift period is flagged more often (9--14\%), most notably by the rank-based predictive $p$-value, but still far less than during the remapping period. In the sparse-population control, HPD overlap and the rank-based predictive $p$-value flag no spikes, whereas KL divergence flags a median of 43\%. This is consistent with the realization shown in Fig~\ref{fig:simulation}a, where HPD overlap and the predictive check registered the spike and the prediction as consistent, while KL responded to the difference in concentration between the predictive distribution and likelihood despite them carrying consistent information about the state process.

The decoding-accuracy rows of Fig~\ref{fig:simulation}b place these flag percentages against ground truth. In the well-specified windows the filtered posterior is accurate and calibrated: the median absolute error is 1.8 a.u.\ and the 95\% HPD region contains the true position at 95\% of time steps. Remapping destroys both (median error 42.2 a.u., coverage 3\%), matching the high flag percentages. History-dependent firing leaves accuracy and calibration essentially intact (1.7 a.u., 94\%), consistent with its low flag percentages: this misfit alters the temporal structure of the spike train without displacing the spatial information each spike carries. The drift period, by contrast, exposes a limitation of event-level diagnostics. The median error rises to 8.0 a.u.\ and coverage falls to 35\%, so the filtered posterior is substantially overconfident about a lagged position, yet only 9--14\% of spikes are flagged. Because each spike is compared with a predictive distribution that the preceding spikes have already pulled toward the lagged estimate, a persistent lag produces a modest per-spike disagreement rather than a large one. During the replay control the error relative to the animal's physical position is large by construction (36.4 a.u., coverage 5\%) because the decoder is tracking the represented trajectory. The sparse-population control is decoded accurately (1.0 a.u., 100\% coverage) even though KL divergence flags 43\% of its spikes, confirming that those flags do not correspond to decoding errors.
