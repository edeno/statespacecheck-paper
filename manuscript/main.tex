...
We first sought to understand how the metrics---KL divergence, HPD overlap, and the predictive $p$-value---perform under common misfit scenarios. This will illustrate the relative strengths of the metrics and what they cannot capture. We therefore evaluated the three metrics on a controlled simulation in which the ground-truth at each moment is known by construction. We simulate an animal moving back and forth on a linear track with neurons that fire selectively for positions on the track (like one would typically see when recording from a hippocampal place-cell population).  We use a state-space decoder over the full session, deliberately perturbing either the generative process or the decoder's assumptions during four distinct windows. These four perturbations were chosen to highlight the distinction between model misfit that leads to decoding errors versus model misfit to which the decoding estimates remain robust. This relates to our original goal of identifying local periods where the information from the current spiking is inconsistent with the aggregated information from our model based on past spiking.

\subsection{Generative Model}\label{sec:generative}

The simulation runs a state space model over a 32-second session whose generative process and decoder coincide everywhere except inside four misfit windows. Each window perturbs one specific component of the matched model.
...
\begin{description}
    ...
    \item[\textbf{History-dependent firing} (steps \num{14000}--\num{18000}; generative side):] spikes are generated with a \SI{1}{ms} hard refractory period followed by a 2--\SI{10}{ms} post-spike burst window in which the rate is multiplied by $3\times$, consistent with the rough phenomenology of CA1 pyramidal-cell refractory and burst windows; the decoder continues to assume independent Poisson firing. The per-spike spatial likelihood for any individual spike is unchanged, so the misfit lives entirely in the spike-train temporal joint distribution.
...
\begin{figure}[htbp]
    ...
    \caption{Per-spike goodness-of-fit diagnostics across a four-misfit simulation. (a) The top block shows the filter posterior probability as a heatmap of time and position. The position is overlaid as a magenta line. The misfit time periods are shaded in color. The second from the top block shows the per-spike likelihood as a heatmap, restricted to time bins containing at least one spike. The position is also overlaid as a magenta line. The third block shows the spike raster with neurons sorted by place-field peak position. The bottom three blocks show the three goodness-of-fit diagnostics (HPD overlap, KL divergence, and predictive $p$-value) for each spike event, with a horizontal dashed line marking the threshold used to flag poor model fit. (b) Shows the percent of spike events that are flagged as poor fit by each diagnostic in each phase of the simulation for each metric as well as comparison to the baseline "clean" phases.}
    \label{fig:simulation}
\end{figure}

Figure~\ref{fig:simulation}a shows a realization of the simulation, the decoding results, and each of the three local goodness-of-fit metrics. The top panel shows the true position of the simulated rat in magenta and the decoding distribution in black. The regions with white backgrounds are the periods when the model is correctly specified and each colored background indicated a period of model misspecification. The first misfit, remapping, is strongly detected by all three diagnostics, with the predictive check showing the strongest response (Figure~\ref{fig:simulation}b). The second misfit, history-dependent firing, is not detected strongly by any of the metrics. This is expected since the position will not change much in the duration of a burst of spiking and so the per-spike likelihood will still be relatively consistent with the predictive distribution unless the state transition matrix were exceedingly narrow and pathalogical. The third misfit, drift, is also only subtly detected by the metrics, with the predictive check showing the strongest response. This is also expected because the drift is relatively slow and the predictive distribution is broad enough to still have substantial overlap with the likelihood. The final misfit, wide-dynamics noise, is strongly detected by the KL divergence but not by the HPD overlap or the predictive check. This is expected because the inflated transition matrix causes the predictive distribution to be much broader than the likelihood, which leads to a large KL divergence but still substantial overlap between the HPD regions and a high rank-based predictive $p$-value.
