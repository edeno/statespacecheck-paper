...
\end{equation}
where $\phi(\cdot \mid \mu, \sigma^{2})$ is the Gaussian density and $\alpha = 5$ is a peak-rate scale factor. With a \SI{1}{ms} simulation step, the peak rate is $\alpha / (\sigma_{\mathrm{pf}}\sqrt{2\pi}) \approx \SI{200}{Hz}$, in the range commonly assumed for active hippocampal pyramidal cells at their place-field centers. Spike counts are drawn independently across cells at each step, $y_{c,t} \sim \mathrm{Poisson}\!\left(\lambda_c(x_t)\right)$.
...
The 38-second simulation ($T = \num{38000}$ steps) is partitioned into ten contiguous phases: an opening baseline, five misfit windows, and four clean-recovery interludes that restore the matched-model configuration. Each misfit perturbs either the generative process or the decoder:
...
\begin{description}
    ...
    \item[\textbf{Wiggly-flat likelihood} (steps \num{34000}--\num{38000}; decoder side):] the decoder swaps its per-cell rate functions for a set of nearly position-independent rates,
    \begin{equation}
    \label{eq:wiggly}
        r_c(x) = r_0 + \delta\, \sin\!\left(2\pi k\, \frac{x}{L} + \varphi_c\right),
    \end{equation}
