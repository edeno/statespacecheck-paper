...
\paragraph{Replay control.} Embedded in the second clean-recovery window (steps \num{19000}--\num{21000}) is a replay event: the animal is immobile while a coherent trajectory sweeps out and back across the track. The decoder faithfully tracks this swept trajectory, so its decoded position departs markedly from the animal's true (fixed) position, yet the model remains internally consistent---the predictive distribution and each spike's likelihood agree throughout the sweep. This is a deliberate positive control for specificity: a large decoded-versus-true discrepancy that is \emph{not} a model misspecification, and that none of the three diagnostics should flag. The replay steps are excluded from the pooled baseline used to set thresholds and are scored as their own column in Fig~\ref{fig:simulation}b.
