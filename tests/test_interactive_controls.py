...
def _build_cache(
    ...
    *,
    ...
    """Build a small synthetic cache for the requested model name."""
    import pandas as pd
    import xarray as xr
    ...
    if n_states == 1:
        state_probs_var = (("time",), np.ones((n_time,), dtype=np.float32))
    else:
        state_probs_var = (
            ("time", "states"),
    ...
    for cell_id, ts in enumerate(spike_times):
        for t_val in ts:
            rows.append((float(t_val), int(cell_id), 0.5, 1.0, 0.5))
